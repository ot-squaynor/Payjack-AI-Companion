...
def _read_batch_cases(batch_file: Path, *, changed_only: bool) -> list[dict[str, Any]]:
...
def _load_cases(batch_files: list[Path], *, changed_only: bool, limit: int | None) -> list[dict[str, Any]]:
    ...
    for batch_file in batch_files:
        cases.extend(_read_batch_cases(batch_file, changed_only=changed_only))
...
def _route_failures(case: dict[str, Any], body: dict[str, Any]) -> list[str]:
    expected_route = case.get("expected_route")
    if expected_route is not None and body.get("route") != expected_route:
        return [f"expected route {expected_route!r} but received {body.get('route')!r}"]
    return []
...
def _tool_trace_failures(case: dict[str, Any], body: dict[str, Any]) -> list[str]:
    tool_traces = body.get("tool_traces") or []
    failures = _presence_failures(
        label="tool_traces",
        value=tool_traces,
...
def _citation_failures(case: dict[str, Any], body: dict[str, Any]) -> list[str]:
    return _presence_failures(
        label="citations",
        value=body.get("citations") or [],
        expectation=case.get("expected_citations"),
    )


def _refusal_failures(case: dict[str, Any], body: dict[str, Any]) -> list[str]:
    ...
    if expected_refusal_category is None and refusal is not None:
        return [f"expected no refusal but received {refusal!r}"]
    if expected_refusal_category is None:
        return []
    if not isinstance(refusal, dict):
        return [f"expected refusal category {expected_refusal_category!r} but received no refusal"]
    if refusal.get("category") != expected_refusal_category:
        return [
...
def _validate_case_response(
    *,
    ...
    failures.extend(_route_failures(case, body))
    failures.extend(
        _debug_failures(
            case=case,
            body=body,
    ...
    failures.extend(_tool_trace_failures(case, body))
    failures.extend(_citation_failures(case, body))
    failures.extend(_refusal_failures(case, body))
    return failures
...
def _prepare_cases(args: argparse.Namespace) -> tuple[list[Path], list[dict[str, Any]]]:
...
def _execute_case(
    *,
    ...
    try:
        ...
        summary = _summarize_case(case=case, failures=case_failures)
        return case_id, summary, case_failures
...
def _run_cases(
    *,
    ...
    for case in cases:
        case_id, summary, case_failures = _execute_case(case=case, args=args)
        summaries.append(summary)
        if case_failures:
            failures_by_case.append((case_id, case_failures))
            if args.fail_fast:
                break
    return summaries, failures_by_case


def _raise_for_failures(failures_by_case: list[tuple[str, list[str]]]) -> None:
...
def main() -> None:
    args = parse_args()
    try:
        batch_files, cases = _prepare_cases(args)
    except Exception as exc:
    ...
    summaries, failures_by_case = _run_cases(cases=cases, args=args)
    print(json.dumps(summaries, indent=2))
    _raise_for_failures(failures_by_case)
